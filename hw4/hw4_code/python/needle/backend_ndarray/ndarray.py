...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod(new_shape) != prod(self._shape):
          raise ValueError('shape product not equal')
        
        if not self.is_compact():
          raise ValueError('matrix not compact')
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        for i, (new, old) in enumerate(zip(new_shape, self._shape)):
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, s in enumerate(slices):
          ...
          new_strides.append(self._strides[i] * s.step)
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        flipped_view = NDArray.make(self.shape, tuple(new_strides), self.device, self._handle, new_offset)
        return flipped_view.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        padded_shape = []
        for i, (head, rear) in enumerate(axes):
          padded_shape.append(self.shape[i] + head + rear)
        padded_shape = tuple(padded_shape)

        out = full(padded_shape, 0, device=self.device, dtype=self.dtype)
        target_slice = []
        for i, (head, _) in enumerate(axes):
          start = head
          end = head + self.shape[i]
          target_slice.append(slice(start, end))
        
        out[tuple(target_slice)] = self
        return out
        ### END YOUR SOLUTION
