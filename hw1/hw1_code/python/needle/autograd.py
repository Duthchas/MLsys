...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      ...
      summed_grad = grad_list[0]
      for i in range(1, len(grad_list)):
        summed_grad = summed_grad + grad_list[i]
      node.grad = summed_grad
      
      if not node.is_leaf():
        input_grads = node.op.gradient_as_tuple(summed_grad, node)
        
        for i, grad_contribution in enumerate(input_grads):
          input_node = node.inputs[i]
          node_to_output_grads_list.setdefault(input_node, []).append(grad_contribution)
    ### END YOUR SOLUTION
