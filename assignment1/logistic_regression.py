...
def logistic_regression(X: ad.Node, W: ad.Node, b: ad.Node) -> ad.Node:
    ...
    """
    matmul_node = ad.matmul(X, W)
    b_broadcast = ad.broadcast_to(b, matmul_node)
    return matmul_node + b_broadcast
...
def softmax_loss(Z: ad.Node, y_one_hot: ad.Node, batch_size: int) -> ad.Node:
    ...
    """
    exp_z = ad.exp(Z)
    sum_exp_z = ad.reduce_sum_axis_one(exp_z)
    log_sum_exp_z = ad.log(sum_exp_z)
    
    z_y = Z * y_one_hot
    sum_z_y = ad.reduce_sum_axis_one(z_y)
    
    loss_per_instance = log_sum_exp_z - sum_z_y
    
    total_loss = ad.sum_op(loss_per_instance)
...
def sgd_epoch(
    ...
    for i in range(num_batches):
        ...
        y_one_hot[np.arange(y_batch.shape[0]), y_batch] = 1.0
    ...
    return W, b, total_loss / num_batches
