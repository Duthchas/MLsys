...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        scaled_scores = scores / (q_dim ** 0.5)
        ...
        probs = self.dropout(self.softmax(scaled_scores))
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        k_proj = self.k_projection(self.prenorm_k(k.reshape((batch_size * queries_len, q_dim))))
        v_proj = self.v_projection(self.prenorm_v(v.reshape((batch_size * queries_len, q_dim))))

        q_ = q_proj.reshape((batch_size, queries_len, self.num_head, self.dim_head)).transpose((1,2))
        k_ = k_proj.reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose((1,2))
        v_ = v_proj.reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose((1,2))
        
        x, _ = self.attn(q_, k_, v_)

        attn_raveled = x.transpose((1,2)).reshape((batch_size * queries_len, self.num_head * self.dim_head))
        result = self.out_projection(attn_raveled).reshape((batch_size, queries_len, self.out_features))
        ### END YOUR SOLUTION
...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        x = (x + self.attn_dropout(attn_output)).reshape((batch_size * seq_len, x_dim))

        norm_x = self.norm(x)
        mlp_output = self.mlp(norm_x)
        x = (x + self.mlp_dropout(mlp_output)).reshape((batch_size, seq_len, x_dim))
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def forward(
        ...
        pos_indices = Tensor(np.arange(seq_len), device=x.device)

        pos_encoding = self.pos_embedding(pos_indices)

        pos_encoding = pos_encoding.reshape((1, seq_len, self.pos_embedding.embedding_dim))
        pos_encoding = pos_encoding.broadcast_to(x.shape)
