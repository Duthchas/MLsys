...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, use_kv_cache=False):
        ...
        if self.flash_attn:
            ...
            # q*kT，（b, self.num_heads, s, self.head_dim）* (b, self.num_heads, self.head_dim，s) = （b, self.num_heads, s, s）
            ...
            mask = torch.full((1, 1, self.config.max_seq_len, self.config.max_seq_len), float("-inf"), device=q.device)
