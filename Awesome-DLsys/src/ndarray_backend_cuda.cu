...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  for (int d = dims - 1; d >= 0; --d) {
    int32_t s = shape.data[d];
    idx[d] = static_cast<int32_t>(tmp % s);
    tmp /= s;
  }
  size_t in_pos = offset;
  for (uint32_t d = 0; d < dims; ++d) {
    in_pos += static_cast<size_t>(idx[d]) * static_cast<size_t>(strides.data[d]);
  }
  out[gid] = a[in_pos];
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  for (uint32_t d = 0; d < dims; ++d) {
    out_pos += static_cast<size_t>(idx[d]) * static_cast<size_t>(strides.data[d]);
  }
  out[out_pos] = a[gid];
}
...
// add this kernel at file scope (e.g., above Matmul)
