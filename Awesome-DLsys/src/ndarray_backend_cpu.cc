...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  for (size_t t = 0; t < total; t++) {
    ...
    for (size_t d = 0; d < ndim; d++) {
      in_pos += static_cast<size_t>(strides[d]) * static_cast<size_t>(idx[d]);
    }
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  for (size_t t = 0; t < total; t++) {
    ...
    for (size_t d = 0; d < ndim; d++) {
      out_pos += static_cast<size_t>(strides[d]) * static_cast<size_t>(idx[d]);
    }
    out->ptr[out_pos] = a.ptr[in_pos++];
    for (int d = static_cast<int>(ndim) - 1; d >= 0; d--) {
      idx[d]++;
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  for (uint32_t it = 0; it < Mt; it++) {
    for (uint32_t kt = 0; kt < Pt; kt++) {
      size_t out_base = (((size_t)it * Pt + kt) * TILE) * TILE;
      float* out_block = out->ptr + out_base;
      for (uint32_t jt = 0; jt < Nt; jt++) {
        size_t a_base = (((size_t)it * Nt + jt) * TILE) * TILE;
        size_t b_base = (((size_t)jt * Pt + kt) * TILE) * TILE;
        const float* a_block = a.ptr + a_base;
        const float* b_block = b.ptr + b_base;
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < groups; i++) {
    ...
    scalar_t mx = a.ptr[base];
    for (size_t j = 1; j < reduce_size; j++) {
      scalar_t v = a.ptr[base + j];
      mx = v > mx ? v : mx;
    }
    out->ptr[i] = mx;
  }
