...
def compare_rope(ax):
    ...
    hf_out = hf_apply_rotary_pos_emb(
        x_t, x_t, cos, sin, None, unsqueeze_dim=2
    )[0].detach().cpu().numpy()
...
def compare_swiglu(ax):
    ...
    layer.ff_linear1.bias = Parameter(
        ndl.Tensor(np.zeros((1, 2 * hidden), dtype=np.float32))
    )
...
def main():
    ...
    compare_gqa(axs[2])
    plt.tight_layout()
    # In notebook/Colab, save to file so the figure can be viewed even if GUI display is unavailable.
    save_path = os.environ.get("SECTION6_SAVE", "section6_visualize.png")
    try:
