...
class BackendDevice:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if "cpu" in names and "cpu_numpy" in names:
            return True
...
class NDArray:
    ...
    def make(
        ...
        if handle is None:
            try:
                ...
                numpy_device = cpu_numpy()
                array._device = numpy_device
                array._handle = numpy_device.Array(prod(shape))
        else:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod(self.shape) != prod(new_shape):
            raise ValueError("Product of dimensions must remain the same.")
        return NDArray.make(
            tuple(new_shape),
            strides=NDArray.compact_strides(tuple(new_shape)),
            device=self.device,
            handle=self._handle,
            offset=self._offset,
        )
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_axes) == self.ndim, "Permutation must specify all axes"
        assert sorted(new_axes) == list(range(self.ndim)), "new_axes must be a permutation"
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        orig_strides = self._strides
        nd = self.ndim
        new_nd = len(new_shape)
        assert new_nd >= nd, "Cannot broadcast to a shape with fewer dimensions"
        out_strides = [0] * new_nd
        for i in range(1, new_nd + 1):
            tgt_dim = new_shape[-i]
            if i <= nd:
                src_dim = orig_shape[-i]
                src_stride = orig_strides[-i]
                if src_dim == tgt_dim:
                    out_strides[-i] = src_stride
                else:
                    assert src_dim == 1, "Incompatible shapes for broadcast"
                    out_strides[-i] = 0
            else:
                out_strides[-i] = 0
        return NDArray.make(
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for dim, sl in enumerate(slices):
            start, stop, step = sl.start, sl.stop, sl.step
            new_offset += start * self._strides[dim]
    ...
    def ewise_or_scalar(
        ...
        out = NDArray.make(self.shape, device=self.device)
        if isinstance(other, NDArray):
            if self.device.name != other.device.name:
                other = NDArray(other.numpy(), device=self.device)
    ...
    def sum(self, axis: int | tuple[int, ...] | list[int] | None = None, keepdims: bool = False) -> "NDArray":
        if isinstance(axis, (tuple, list)):
            ...
            res: NDArray = self
            for ax in axes_norm:
                res = res.sum(ax, keepdims=True)
            if not keepdims:
                target_shape = [res.shape[i] for i in range(res.ndim) if i not in axes_norm]
                if len(target_shape) == 0:
                    target_shape = (1,)
                res = res.reshape(tuple(target_shape))
            return res
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        for ax in axes:
            if ax < 0:
                ax += self.ndim
            assert 0 <= ax < self.ndim, "Axis out of bounds"
            if ax not in axes_norm:
                axes_norm.append(ax)
        ...
        new_offset = self._offset
        for ax in axes_norm:
            stride = new_strides[ax]
            new_offset += (self._shape[ax] - 1) * stride
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        if not isinstance(axes, (tuple, list)):
            raise TypeError("axes must be a sequence of (before, after) tuples")
        if len(axes) != self.ndim:
            raise ValueError("axes length must match array ndim")

        pad_pairs = [tuple(p) for p in axes]
        for p in pad_pairs:
            if len(p) != 2:
                raise ValueError("Each padding specification must be length 2")
            if p[0] < 0 or p[1] < 0:
                raise ValueError("Padding values must be non-negative")

        new_shape = tuple(
            self.shape[i] + pad_pairs[i][0] + pad_pairs[i][1] for i in range(self.ndim)
        )
        out = NDArray.make(new_shape, device=self.device)
        out.fill(0.0)

        slices = tuple(
            slice(pad_pairs[i][0], pad_pairs[i][0] + self.shape[i])
...
def divide(a: NDArray, b: NDArray | float) -> NDArray:
    return a / b

def power(a: NDArray, b: NDArray | float) -> NDArray:
...
def swapaxes(a: NDArray, ax1: int, ax2: int) -> NDArray:
