...
class BatchNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
          ...
          mean_reshape = mean.reshape((1, self.dim)).broadcast_to(x.shape)

          var = ops.summation((x - mean_reshape) ** 2, axes = (0,)) / x.shape[0]
          ...
          normalized_x = x - mean_reshape
          div_part = (var.broadcast_to(x.shape) + self.eps) ** 0.5

          return self.weight.reshape((1, self.dim)).broadcast_to(x.shape) * normalized_x / div_part + \
          ...
          div_part = (self.running_var.reshape((1, self.dim)).broadcast_to(x.shape) + self.eps) ** 0.5
