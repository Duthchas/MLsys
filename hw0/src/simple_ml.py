...
def softmax_loss(Z, y):
    ...
    correct_class_probabilities = P[np.arange(batch_size), y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
      ...
      I_y = np.zeros((current_batch_size, num_classes))
      I_y[np.arange(current_batch_size), y_batch] = 1

      gradient = (1 / current_batch_size) * (X_batch.T @ (Z - I_y))
